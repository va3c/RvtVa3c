...
namespace RvtVa3c
{
  internal class MattMasonMaterial
  {
    #region Declarations
    ...
    #region Constructors
    internal MattMasonMaterial( Material m )
    {
      Id = m.Id;
      Name = m.Name;
      ...
      List<AssetPropertyString> images = Properties.Values
        .Where( p => p.Type == AssetPropertyType.APT_String )
        .Cast<AssetPropertyString>()
        .Where( p => p.Value.ToUpper().EndsWith( ".PNG" ) )
        .ToList();
    }
    #endregion
